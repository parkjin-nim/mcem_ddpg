...
class CriticNetwork(nn.Module):
    def __init__(self, beta, input_dims, fc1_dims, fc2_dims, n_actions,
                 name, chkpt_dir='tmp/td3_400x300'):
...
class ActorNetwork(nn.Module):
    def __init__(self, alpha, input_dims, fc1_dims=64, fc2_dims=64,
                 n_actions=None, name=None, chkpt_dir='tmp/td3_400x300'):
...
def main():
    ...
    filename = 'TD_400x300_' + env_id + '_' + str(n_games) + '_games' + '4'
    figure_file = 'plots/' + filename + '.png'
    np.save("npy/" + filename + ".npy", np.array(score_history))
    plotLearning(score_history, figure_file, wait=wait)
